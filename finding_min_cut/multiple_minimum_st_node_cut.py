...
from minimum_st_edge_cut import minSTEdgeCut
from min_cut_utility import BFS_Thru_Zero
...
def multiple_minimum_st_node_cut(G, source, sink) :
	...
	paths, single_st_edge_cut = zero_capacity_paths_from_residual(G, source, sink)
...
def zero_capacity_paths_from_residual(G, source, sink):
	R, single_st_edge_cut = minSTEdgeCut(G, source, sink)
	...
	while BFS_Thru_Zero(R, source, sink, parent) :
		v = sink
		path = []
		temp = []
		temp.append(v)
	...
	print 'whole_residual_paths ' + str(temps)

	return paths, single_st_edge_cut
